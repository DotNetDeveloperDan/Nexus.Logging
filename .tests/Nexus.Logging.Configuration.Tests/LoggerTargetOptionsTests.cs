...
namespace Nexus.Logging.Configuration.Tests
{
    public class LoggerTargetOptionsTests
    {
        ...
        [TestCase("PathFormat", @"\defaultlogs\log.log", @"\logs\testing.log", Description = "Match should be found with return type string")]
        [TestCase("FileSize", 15000000L, 10000000L, Description = "Match should be found with return type long")]
        [TestCase("Unmatched", true, true, Description = "Default should be returned since key does not exist with return type bool")]
        public void When_GettingValueFromVals_Should_GetValueWhenExistsOrDefaultWhenNotExists<T>(string key, T defaultValue, T expected)
        {
            var result = _testArgs.GetValueOrDefault<T>(key, defaultValue);
